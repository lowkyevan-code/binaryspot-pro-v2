      ...
      <div className="max-w-[1600px] mx-auto lg:flex lg:min-h-[calc(100vh-132px)]">
        ...
        {activeTab ===
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-3xl border border-slate-800 bg-[#090e16] overflow-hidden">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-800 bg-gradient-to-r from-emerald-500/10 via-cyan-500/5 to-transparent p-5">
                <div>
                  ...
                  <p className="mt-2 max-w-2xl text-xs leading-5 text-slate-500">
                    Configure the market, strategy, trade options and safety limits in one guided workspace.
                  </p>
                </div>

                <span className={`rounded-xl border px-3 py-2 text-[10px] font-black ${
                  isAutoBotRunning
                ...
                <div className="space-y-4 p-4 sm:p-5">
                  <div className="rounded-2xl border border-slate-800 bg-[#0d131d] p-4">
                    <div className="mb-4 flex items-center gap-3">
                      <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-emerald-500 text-xs font-black text-black">1</span>
                      <div>
                        <p className="text-sm font-black">
                          Trade parameters
                        </p>
                        <p className="text-[10px] text-slate-600">
                          Market, BinarySpot strategy and execution contract
                        </p>
                      </div>
                    </div>

                    <div className="grid sm:grid-cols-2 gap-4">
                      <Field label={`Market ${activeSymbolsStatus === 'ready' ? `• ${activeSymbols.length} live` : ''}`}>
                        <select
                          value={symbol}
                          onChange={(event) =>
                            changeSymbol(
                      ...
                      <Field label="Supported Trade Type">
                        <select
                      ...
                      <Field label="Supported Contract">
                        <select
                          ...
                          disabled={
                            ...
                            isContractOpen ||
                            pendingBuyStatus.blocking ||
                            builderContracts.length ===
                              0
                          }
                          className={INPUT_CLASS}
                        >
                          {builderContracts.length ? (
                            builderContracts.map(
                              (contract) => (
                                <option
                                  key={`${contract.contract_type}-${contract.sentiment || ''}-${contract.expiry_type || ''}`}
                                  value={
                                    contract.contract_type
                                  }
                                >
                            ...
                            <option value="">
                              No supported contracts
                            </option>
                          )}
                        </select>
                      </Field>

                      <Field label="BinarySpot Strategy">
                        <select
                          value={strategy}
                          onChange={(event) =>
                            selectStrategy(
                      ...
                      <Field label="Strategy Execution">
                        <input
                          value={
                            strategyExecutionContract ||
                            'WAIT'
                      ...
                      <Field label="Prediction / Barrier">
                        <input
                          value={
                            signalExecution.predictionDigit !== null &&
                            signalExecution.predictionDigit !== undefined
                    ...
                    <div className={`mt-4 rounded-xl border p-3 ${
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                        <div>
                          <p className="text-[10px] uppercase tracking-wider font-black text-slate-500">
                            Deriv Contract Compatibility
                          </p>
                          <p className="mt-1 text-xs font-black text-slate-300">
                            {contractsStatus !== 'ready'
                              ? 'Checking contracts supported by this market...'
                              : !strategyContractSupported
                              ? `${strategyExecutionContract} is not supported by ${currentMarket?.underlying_symbol_name || symbol}.`
                              : !selectedContractMatchesStrategy
                              ? `Selected ${activeBuilderContractType} differs from the current strategy execution ${strategyExecutionContract}.`
                              : `Current strategy execution is supported on ${currentMarket?.underlying_symbol_name || symbol}.`}
                          </p>
                        </div>
                      ...
                      {selectedBuilderContract && (
                        <div className="mt-3 flex flex-wrap gap-2">
                          {selectedBuilderContract.expiry_type && (
                            <span className="rounded-lg border border-slate-800 bg-slate-900 px-2 py-1 text-[9px] font-black text-slate-500">
                              {String(selectedBuilderContract.expiry_type).toUpperCase()}
                            </span>
                          )}
                          {selectedBuilderContract.sentiment && (
                            <span className="rounded-lg border border-slate-800 bg-slate-900 px-2 py-1 text-[9px] font-black text-slate-500">
                              {String(selectedBuilderContract.sentiment).toUpperCase()}
                            </span>
                          )}
                          {selectedBuilderContract.market && (
                            <span className="rounded-lg border border-slate-800 bg-slate-900 px-2 py-1 text-[9px] font-black text-slate-500">
                              {String(selectedBuilderContract.market).toUpperCase()}
                            </span>
                          )}
                        </div>
                      )}
                    ...
                    <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
                      <MiniMetric
                        label="Market"
                        value={currentMarket?.underlying_symbol_name || symbol}
                      />
                      <MiniMetric
                        label="Contracts"
                        value={
                      ...
                      <MiniMetric
                        label="Confidence"
                        value={`${Number(signal.confidence || 0).toFixed(1)}%`}
                      />
                    </div>
                  </div>
                  ...
                  <div className="rounded-2xl border border-slate-800 bg-[#0d131d] p-4">
                    <div className="mb-4 flex items-center gap-3">
                      <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-cyan-500 text-xs font-black text-black">2</span>
                      <div>
                        <p className="text-sm font-black">
                          Trade options
                        </p>
                        <p className="text-[10px] text-slate-600">
                          Duration, stake and minimum signal confidence
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="rounded-2xl border border-slate-800 bg-[#0d131d] p-4">
                    <div className="mb-4 flex items-center gap-3">
                      <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-violet-500 text-xs font-black text-white">3</span>
                      <div>
                        <p className="text-sm font-black">
                          Risk management
                        </p>
                        <p className="text-[10px] text-slate-600">
                          Session limits and loss protection
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="rounded-2xl border border-slate-800 bg-[#0d131d] p-4">
                    <div className="mb-4 flex items-center gap-3">
                      <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-amber-400 text-xs font-black text-black">4</span>
                      <div>
                        <p className="text-sm font-black">
                          Recovery controls
                        </p>
                        <p className="text-[10px] text-slate-600">
                          Stake multiplier, stake cap and cooldown
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="grid sm:grid-cols-3 gap-4">
                      ...
                      <Field label={`Maximum Stake (${currency})`}>
                        <input
                          type="number"
                          min="0.35"
                          step="0.01"
                    ...
                    <p className="mt-3 text-[10px] leading-5 text-slate-600">
                      Stake progression cannot create a statistical edge. Existing BinarySpot loss, stake and session safety limits remain active.
                    </p>
                  </div>
                </div>

                <div className="border-t xl:border-l xl:border-t-0 border-slate-800 bg-[#070b11] p-4 sm:p-5">
                  ...
                  <p className="mt-2 text-xs leading-5 text-slate-500">
                    {selectedStrategy?.description || signal.reason}
                  </p>
                  ...
                  <div className="mt-5 space-y-3">
                    ...
                    <BuilderSummaryRow
                      label="Duration"
                      value={`${duration} tick${Number(duration) === 1 ? '' : 's'}`}
                    />
                    <BuilderSummaryRow
                      label="Base Stake"
                      value={`${baseStake} ${currency}`}
                    />
                    <BuilderSummaryRow
                      label="Current Stake"
                      value={`${currentStake} ${currency}`}
                    />
                    <BuilderSummaryRow
                      label="Max Trades"
                      value={maxTrades}
                    ...
                    <BuilderSummaryRow
                      ...
                      value={
                        accountType
                          ? `${accountType.toUpperCase()} • ${accountId || '—'}`
                          : 'OFFLINE'
                      }
                    />
                  </div>

                  <div className="mt-5 rounded-2xl border border-slate-800 bg-[#0b111a] p-4">
                    <div className="flex items-center justify-between gap-3">
                      <span className="text-[10px] font-black text-slate-500">
                        SIGNAL QUALITY
                      </span>
                      <span className="text-sm font-black text-emerald-400">
                        {Number(signal.confidence || 0).toFixed(1)}%
                      </span>
                    </div>
                    ...
                    <p className="mt-3 text-[10px] leading-5 text-slate-600">
                      Confidence measures current signal quality; it is not a guaranteed win probability.
                    </p>
                  </div>
                  ...
                  <button
                    ...
                    className={`mt-5 w-full rounded-xl px-4 py-4 text-sm font-black transition disabled:opacity-40 ${
                      isAutoBotRunning
                  ...
                  {contractsStatus === 'ready' &&
                    !strategyContractSupported && (
                    <p className="mt-3 text-center text-[10px] font-black text-amber-300">
                      THIS STRATEGY CONTRACT IS NOT SUPPORTED BY THE SELECTED MARKET
                    </p>
                  )}
                  ...
                  <button
                    ...
                    className="mt-3 w-full rounded-xl border border-slate-800 bg-slate-900 px-4 py-3 text-[10px] font-black text-slate-400"
                  >
                    OPEN BACKTESTING
                  </button>
                </div>
          ...
          <div className="mx-auto max-w-5xl space-y-4">
            <div className="overflow-hidden rounded-2xl border border-slate-800 bg-[#07090c] shadow-2xl">
              ...
              <div className="p-3 sm:p-5">
                <div className="mb-4 overflow-hidden rounded-xl border border-slate-800 bg-[#080b10]">
                  <div className="flex items-center justify-between border-b border-slate-800 px-4 py-3">
                    <div>
                      <p className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-400">Live Deriv chart</p>
                      <p className="mt-1 text-sm font-black text-white">{currentMarket?.underlying_symbol_name || symbol}</p>
                    </div>
                    <button type="button" onClick={() => setActiveTab('chart')} className="rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-[10px] font-black text-slate-300">EXPAND</button>
                  </div>
                  <div className="h-[300px] sm:h-[380px]">
                    <DerivSmartChartPanel symbol={symbol} currentMarket={currentMarket} />
                  </div>
                </div>
...
const SmartChart = dynamic(
    ...
    }),
  {
    ssr: false,
    loading: () => (
      <div className="flex h-full min-h-[280px] items-center justify-center bg-[#080b10] text-sm text-slate-500">
        Loading Deriv chart…
      </div>
    ),
  }
...
function DerivSmartChartPanel({ symbol, currentMarket }) {
  const adapterRef = useRef(null);

  if (!adapterRef.current && typeof window !== 'undefined') {
    adapterRef.current = createDerivSmartChartsAdapter();
  }

  useEffect(() => {
    const stylesheetId = 'binaryspot-smartcharts-css';
    if (!document.getElementById(stylesheetId)) {
      const link = document.createElement('link');
      link.id = stylesheetId;
      link.rel = 'stylesheet';
      link.href = '/smartcharts/smartcharts.css';
  ...
  return (
    <div className="h-full w-full bg-[#080b10]">
      <SmartChart
        key={symbol}
        id={`binaryspot-${symbol}`}
        symbol={symbol}
        chartType="candle"
        granularity={60}
        ...
        chartData={{
          activeSymbols: [
            {
              symbol,
              display_name: currentMarket?.underlying_symbol_name || symbol,
              market: currentMarket?.market || 'synthetic_index',
              market_display_name: currentMarket?.market_display_name || 'Derived',
              submarket: currentMarket?.subgroup || currentMarket?.submarket || 'continuous_indices',
              submarket_display_name: currentMarket?.submarket_display_name || 'Continuous Indices',
              pip: currentMarket?.pip_size ? 10 ** -Number(currentMarket.pip_size) : 0.01,
              exchange_is_open: 1,
...
function DerivChartWorkspace({
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-800 bg-[#080b10] shadow-2xl">
      <div className="flex flex-col gap-3 border-b border-slate-800 bg-[#0d1117] p-3 sm:flex-row sm:items-center sm:justify-between sm:p-4">
        <div className="min-w-0">
          <p className="text-[10px] font-black uppercase tracking-[0.22em] text-emerald-400">Deriv live market</p>
          <select
            value={symbol}
            onChange={(event) => changeSymbol(event.target.value)}
            className="mt-1 max-w-full cursor-pointer appearance-none bg-transparent pr-8 text-lg font-black text-white outline-none sm:text-xl"
          >
            {Object.entries(groupedMarkets).map(([group, items]) => (
              <optgroup key={group} label={group}>
                {items.map((item) => (
                  <option key={item.underlying_symbol} value={item.underlying_symbol} className="bg-slate-950">
                    {item.underlying_symbol_name}
                  </option>
                ))}
              </optgroup>
        ...
        <div className="flex items-center gap-4 rounded-xl border border-slate-800 bg-black/30 px-4 py-2.5">
          <div>
            <p className="text-[9px] font-black uppercase tracking-wider text-slate-600">Price</p>
            <p className="font-mono text-base font-black text-white">{displayQuote}</p>
          </div>
          <div className="h-8 w-px bg-slate-800" />
          <div>
            <p className="text-[9px] font-black uppercase tracking-wider text-slate-600">Last digit</p>
            <p className="text-base font-black text-cyan-400">{lastDigit ?? '—'}</p>
          </div>
          <span className={`h-2.5 w-2.5 rounded-full ${isMarketConnected ? 'bg-emerald-400' : 'bg-rose-400'}`} />
        </div>
      </div>

      <div className="h-[430px] sm:h-[560px] lg:h-[640px]">
        <DerivSmartChartPanel symbol={symbol} currentMarket={currentMarket} />
      </div>

      <div className="flex items-center justify-between border-t border-slate-800 bg-[#0b0f14] px-4 py-3 text-[10px] font-black uppercase tracking-wider text-slate-500">
        <span>SmartCharts • 1 minute candles</span>
        <span className={isMarketConnected ? 'text-emerald-400' : 'text-amber-400'}>
          {isMarketConnected ? 'Market live' : 'Connecting'}
        </span>
      </div>
    </div>
  );
